...
class TestKsefClient:
    ...
    async def test_send_invoice_success(
        self, ksef_client, mock_httpx_client, sample_xml_invoice
    ):
        ...
        send_response.json = lambda: {"ksef_number": "KSEF:2025:PL/1234567890/ABC123"}
    ...
    async def test_get_status_success(
        self, ksef_client, mock_httpx_client, test_ksef_number
    ):
    ...
    async def test_download_success(
        self, ksef_client, mock_httpx_client, test_ksef_number, tmp_path
    ):
        ...
        with patch("pathlib.Path.write_bytes") as mock_write:
            result_path = await ksef_client.download(
                test_ksef_number, output_path=output_path
            )
